...
func (db *DB) GetSettings(userID int) (models.Settings, error) {
	...
	err = json.Unmarshal([]byte(settingsJSON), &settings.Settings)
	if err != nil {
		return settings, err
	}
...
func (db *DB) SaveSettings(settings models.Settings) error {
	...
	_, err = db.Exec("INSERT OR REPLACE INTO settings (user_id, settings) VALUES (?, ?)", settings.UserID, string(settingsJSON))
	return err
}
